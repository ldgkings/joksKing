...
#define DGImageRidio 0.7

@interface DGTabBarButton ()

@property (nonatomic, weak) DGBadgeView *badgeView;

@end

@implementation DGTabBarButton

// 重写setHighlighted，取消高亮做的事情
- (void)setHighlighted:(BOOL)highlighted{}

// 懒加载badgeView
- (DGBadgeView *)badgeView
{
    if (_badgeView == nil) {
        DGBadgeView *btn = [DGBadgeView buttonWithType:UIButtonTypeCustom];
        [self addSubview:btn];
        _badgeView = btn;
    }
    return _badgeView;
...
- (void)layoutSubviews
{
    ...
    CGFloat imageH = self.bounds.size.height * DGImageRidio;
